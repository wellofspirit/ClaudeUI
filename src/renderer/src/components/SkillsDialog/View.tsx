...
export interface SkillsDialogViewProps {
  skills: SkillInfo[]
  loading: boolean
  cwd: string | null
  onClose: () => void
...
export function SkillsDialogView({
  ...
  // Auto-select the first skill when the list loads, and re-select if the
  // previously-selected skill disappears after a cwd change.
  useEffect(() => {
    if (skills.length === 0) return
    const stillPresent = selected !== null && skills.some((s) => s.name === selected)
    if (!stillPresent) {
