...
const components = {
  ...
  code: ({ children, className }: { children?: React.ReactNode; className?: string }) => {
    if (!className) {
      return (
        <code className="bg-bg-tertiary px-1 py-px rounded text-[12px] font-mono text-accent">
          {children}
        </code>
      )
    }
  ...
  h1: ({ children }: { children?: React.ReactNode }) => (
    <h1 className="text-[15px] font-semibold mb-2 mt-3 first:mt-0">{children}</h1>
  ),
  h2: ({ children }: { children?: React.ReactNode }) => (
    <h2 className="text-[14px] font-semibold mb-1.5 mt-2.5 first:mt-0">{children}</h2>
  ),
  ...
  th: ({ children }: { children?: React.ReactNode }) => (
    <th className="border-b border-border px-3 py-1.5 text-left text-[11px] font-semibold text-text-secondary">
      {children}
    </th>
  ),
