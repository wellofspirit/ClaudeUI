...
interface Props {
   ...
   * An affordance hint, not a verdict — the server's refusal codes below move
 ...
 * and client-cacheable, which is precisely the property that makes decay
  ...
  const apply = useCallback(
    (result: TerminalStepUpResult): void => {
      ...
      if (result.code === 'passkey-required') {
        setMode('passkey')
        setPassword('')
        setError(result.error ?? 'This server requires a passkey to unlock the terminal.')
        return
      }
  ...
  return (
    ...
    >
      ...
      <div className="text-[11px] text-text-muted max-w-[380px] leading-snug">
        {mode === 'passkey'
          ? 'Confirm with your passkey to open a shell on the host. Access ends automatically after a period of inactivity.'
          : 'Enter your remote-access password to open a shell on the host. Access ends automatically after a period of inactivity.'}
      </div>

      {mode === 'passkey' ? (
        <button
          data-testid="TerminalStepUpPrompt.passkey"
          disabled={busy}
      ...
      {passkey && (
        <button
          data-testid="TerminalStepUpPrompt.switchFactor"
          onClick={() => {
            setError(null)
