...
// ── Codex mark ────────────────────────────────────────────────────────────────
// Codex logomark glyph (lobehub codex-color), gradient fill, no background tile.

function CodexMark({ size, className }: { size: number; className: string }): React.JSX.Element {
  // Unique gradient id per instance — many marks render at once (one per Codex
  // session row + the TopBar) and duplicate SVG ids break gradient references.
