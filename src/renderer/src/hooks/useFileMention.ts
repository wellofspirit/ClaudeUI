...
export function useFileMention({
  ...
  useEffect(() => {
    ...
    window.api
      .listDir(fullDir)
      .then(({ entries, isRoot, resolvedPath }) => {
  ...
  useEffect(() => {
    ...
    const after = text.slice(
      fileMentionAnchor + 1 + oldDirWithSlash.length + fileMentionQuery.length
    )
  ...
  const handleNavigate = useCallback(
    (entry: DirEntry): void => {
      ...
      const after = text.slice(
        fileMentionAnchor +
          1 +
          (fileMentionDir ? fileMentionDir.length + 1 : 0) +
          fileMentionQuery.length
      )
  ...
  const handleInputChange = useCallback(
    (value: string, cursorPos: number): void => {
      if (fileMentionOpen) {
