 ...
 *
 * This module now lives inside the Electron-free `src/core` fence (S2 physical
 * extraction): the window handle it delivers host-local channels to is the
 * neutral `HostWindowHandle` from `core/host.ts` (a real `BrowserWindow` on the
 * desktop, `null`/absent headless), never an Electron import. It owns:
 *
...
function sendToWindow(win: HostWindowHandle | null, channel: string, args: unknown[]): void {
 ...
 */
export function emitEvent(channel: string, args: unknown[], win?: HostWindowHandle | null): void {
