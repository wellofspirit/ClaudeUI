...
**Minified names are only scope-unique.** The darwin-arm64 2.1.280 bundle names the helper `rme` and _also_ defines an unrelated `async function rme(` (an MCP config loader) in another chunk; the Windows bundle happened not to collide. The helper is therefore selected by content — the `async function NAME(a,b){let{taskRegistry:` definition — and must be the only such definition; a plain "signature appears once" check made the upstream detection fall through to the legacy path and abort on macOS.
