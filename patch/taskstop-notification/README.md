...
2. **~~"killed" status rejected~~** — _(Fixed upstream in SDK 0.2.49)_ The CLI uses `"killed"` internally, but the XML parser's validator only accepts `completed|failed|stopped`. Unknown statuses default to `"completed"`, so stopped tasks would show as completed.
