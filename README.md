...
- **Native Claude Code integration, no SDK.** ClaudeUI rebundles Anthropic's official standalone binary with a set of content-anchored patches, then speaks the stream-json wire protocol directly from an in-house harness. The patches unlock what the stock CLI doesn't expose: live subagent streaming, unbuffered bash output, voice input, and file-based credentials. Everything else runs on the CLI's own wire surfaces, so chat also works on Anthropic's unpatched binary; there the streaming falls back to complete messages and voice is off. The full protocol is documented in [`docs/protocol-cc/`](docs/protocol-cc/).
