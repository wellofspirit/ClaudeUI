...
- **Nullable cost + opencode target approval id (`ef83a639`)**: `SessionStatus`/`StatusLineData`/`AutomationRun.totalCostUsd` are `number | null`; renderers show the word unknown for null; Codex reports the equivalent or null; the opencode dispatch target's forwarded approvals carry the tool part's callID so they bind inline as well as floating.
- **Lineage cache + interrupted-tool probe (`cf11b983`)**: db v17 turns `codex_forks` into a lineage cache with roots and tombstones; a launch-time scan reads only unknown/changed ids; the delete plan is cache-only with one rescan on refusal; the interrupted hosted-tool call is pinned absent from cold history and result-less live.
- **Interrupted hosted-tool tombstone (`6f69d427`)**: after an interrupted or failed turn, every hosted tool_use without a result gets a synthesized `isError` result (`[Request interrupted by user for tool use]`, or a stopped variant for a failed turn or lost transport); cold history unchanged by decision.
