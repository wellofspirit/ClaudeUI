...
**What it is NOT:** it is **not** in `bun run test` or `bun run test:ci` — it needs a browser binary and takes seconds per file, so locally it is gated exactly like `integration`. CI runs it as its own `layout` job on `windows-2022` (`.github/workflows/ci.yml`): the tier thresholds were measured on win32, so that is the platform where a failure means something, and the job installs with `--ignore-scripts` because the project imports renderer code only. An assertion here must therefore be RELATIONAL (this fits inside that; this shrinks by what left minus what arrived), never an absolute pixel count — text is as wide as the host's fonts make it. And it measures **Playwright's Chromium, not the shipped Electron build**, which runs a different Chromium version; a claim that depends on engine-specific behaviour has to hold in both readings (see the `container-type` stacking-context case in `TopBar.layout.test.tsx`) rather than being settled by what this browser happens to do.
