...
| `scripts/extract-cli.mjs` | Downloads the per-platform Bun binary (SHA-verified against the manifest), pulls `cli.js` out of its `.bun`/`__BUN` section. Cache-hit skip when `version.json#version` matches `package.json#claudeCliVersion` AND `form === 'wrapped'`. |
...
`extract-cli.mjs` compares `package.json#claudeCliVersion` against `vendor/claude-cli/version.json#version` AND requires `form === 'wrapped'`. Match → skip everything. Mismatch (including the cross-pipeline migration from unwrapped to wrapped form) → full re-download + extract.

`bun run ensure-cli` is the invocation; it's gated into `postinstall`, `dev`, `build`, `build:mac`, `build:linux`. Cache-warm pipeline (extract + patch + rebundle) runs in ~4 s; `rebundle-cli.mjs` alone is ~270 ms. Full cold extract adds the ~60 s binary download.
