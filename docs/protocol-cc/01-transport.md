...
**`flags` and the Bun 1.4 optional records.** Bun 1.4 chains optional records *after* the module table, each gated by a `flags` bit (`Flags` in `StandaloneModuleGraph`):
