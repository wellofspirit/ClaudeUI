...
**2026-09-25 (ADR-077):** the patch counts above predate the patch-set reduction. The registry now holds five patches — `subagent-streaming`, `voice-server`, `bash-output-streaming`, `subprocess-proxy-strip`, `skip-securestorage` (01 §1.12) — and `bun run test:patch` runs four suites (`voice-server` has none). Of the other nine, five were deleted as dead or upstream and four replaced by native surfaces, each documented here against the 2.1.280 bundle: `background_tasks`, `cancel_async_message` and `reload_plugins` (07), `command_lifecycle` (03 §3.21), `rate_limit_event.rate_limit_info.unifiedWindows` (03 §3.11). The app reads the patches a binary carries from `version.json` `patches` (01 §1.12) and runs on Anthropic's unpatched binary with subagent token streaming and live Bash output absent and voice off.
