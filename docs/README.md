...
- [architecture/](architecture/README.md) — how the app is put together, split by concern: source layout + services, the multi-engine seam, IPC & data flow, persistence/settings planes, UI, the remote layer as built ([remote.md](architecture/remote.md)), and the accepted target designs for sync ([sync-core.md](architecture/sync-core.md)) and remote security ([security.md](architecture/security.md)). **Start at its README for any structural question.**
