...
4c's second half closed the last of it: the desktop store's replicated slices are now `applyEvent`'s output, projected by `renderer/src/stores/replica.ts`, so `useClaudeEvents` and the reducer are no longer two interpretations of one stream — there is one, and the shadow comparator that measured the gap between them is deleted. The registry-config mutations (pins, titles, recents, `sessionEngines`, settings) are replica-owned too; they still apply locally before their `config:*` echo, deliberately, because the save merges from current state — the reasoning is in [sync-core.md](sync-core.md)'s deviation note, and turning them into real commands belongs with the `command()` registry rather than with the store split.
...
>
> The other 4c half — the renderer adopting the shared reducer — **is done too.** The
> desktop store's replicated slices are `applyEvent`'s output projected by
> `renderer/src/stores/replica.ts`; ~40 store actions and every per-channel handler for a
> `canonical: true` channel are deleted, and the shadow comparator retired with them.
> **Every defect in the ledger below is now closed except the one sentence noted under
> defect 5**, which is a phase-5 concern rather than a 4c one.

1. **Interactions are RPCs, not events.** Whether other clients learn of a mutation depends on the handler: `sendPrompt` fans out; `dequeueMessage` broadcasts nothing (`handlers-core.ts`); `set-model`/`set-effort`/`set-thinking-mode` emit nothing any client maps back into picker state; `setPermissionMode` is the only per-session config with a full bidirectional path. — *FIXED: 4a added `session:config-changed` (a partial per-field replace, emitted pre-spawn too) and ADR-053 made the queue itemized and event-driven; 4c made every client APPLY those events through the shared reducer instead of its own handler, and deleted the optimistic write + revert that used to paper over the gap. What is NOT done is the generic `command()` registry — the mutations are still invokes — which is why the registry-config writes apply locally before their echo (see sync-core.md's deviation note).*
2. **Privileged desktop renderer.** Snapshot = `window.__getRemoteState()`; a busy/hung/missing renderer silently yields an **empty** snapshot; a remote client's own state (model pick, queued display) is clobbered by the desktop's ignorance on resync (ADR-041 merge: snapshot wins per known session). — *FULLY FIXED: state-of-record half in 4b (canonical snapshots), delivery-privilege half in 4c (the desktop is a subscriber, not a target). `__getRemoteState` and `getRemoteStateSnapshot` are DELETED as of 4c's reducer adoption — the desktop store is a replica like any other client's, and the property is asserted the other way round now (`sync-hydration-parity.e2e.test.ts`: a fresh replica from `core.getSnapshot()` equals the live one).*
...
4. **Fire-and-forget event apply:** `api-adapter` drops events with no registered listener while `connection.ts` still advances `lastSeq` — events arriving before React mounts (every phone foreground) are acked but never applied. — *FIXED in phase 0 (the readiness gate + pre-mount buffer, `lastSeq` advancing only on dispatch), and 4c narrowed the window further: the replica installs its tap before React mounts, so the fold no longer waits on a component tree at all.*
...
6. **Queue ghosts:** the queued-message display is a `\n`-joined string per session; dequeue matches by full text against cli.js's per-item queue (no item ids), so with 2+ queued items removal always misses (`removed: 0`) — and the UI clears anyway, leaving invisible messages that execute later. Dequeue is never broadcast; queue state is absent from the snapshot. — *FIXED in phase 3 (ADR-053: itemized queue of record, every transition an event, `queue` in the snapshot). 4c removed the last copy of its apply logic from the renderer, so the `steer-<itemId>` synthesis every replica performs is now literally one function.*
