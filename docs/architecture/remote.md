...
**Phase 4b changed the sentence that used to matter most here.** The desktop renderer is no longer the state of record: full-state snapshots are canonical state in the main process, folded from the same event stream every client replays, so state the desktop happens not to know about is no longer erased from remote clients at their next sync-full. What remains of the old shape is *delivery* privilege — the desktop window is still a distinguished fan-out target with an `extras-only` asymmetry — and the renderer's store is still its own hand-written replica rather than a reducer consumer. Both die in 4c, at which point this file's framing stops being "IPC mirror" at all.
...
>
> **The delivery-privilege half of defect 2 still stands:** the desktop window is a
> distinguished fan-out target and `extras-only` call sites still exist. That dies in
> 4c (see sync-core.md's stage table).
...
2. **Privileged desktop renderer.** Snapshot = `window.__getRemoteState()`; a busy/hung/missing renderer silently yields an **empty** snapshot; a remote client's own state (model pick, queued display) is clobbered by the desktop's ignorance on resync (ADR-041 merge: snapshot wins per known session). — *state-of-record half FIXED in 4b (canonical snapshots); delivery-privilege half open until 4c.*
