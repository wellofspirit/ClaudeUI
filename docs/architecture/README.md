...
How ClaudeUI is put together. This directory splits the architecture by concern — read the file that matches your task. Design rationale lives in [`docs/adr/`](../adr/adr.md); the cli.js wire protocol in [`docs/protocol-cc/`](../protocol/README.md).
...
| [codex.md](codex.md)                 | Codex as built: transport, session adapter, the shared permission model (ADR-067), catalog/effort, native auth and the history baseline, plus the planned queue/tools/children/fork/metering work under ADR-066  |
...
| [metering.md](metering.md)           | **Metering** as built — the usage ledger, two costs per row, machine-independent account keys, hourly buckets and the window ledger, the dashboard, and the opt-in sync to a self-hosted usage hub (ADR-071/072) |
