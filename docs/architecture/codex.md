...
| `CodexHost` / `CodexHostRegistry`  | ONE app-server per Codex home and injected account (ADR-069 §1): lazy single-flight start, the injected identity and its refresh answer, leases for readers, the idle deadline, graceful close, and removal on death                                                                                                      |
