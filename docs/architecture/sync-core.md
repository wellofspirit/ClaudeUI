...
| 2 | **Terminal** (PTY manager, multi-attach, step-up, audit) | M | Shell usable from web behind opt-in + step-up | **landed** (`0e60c7e`, 2026-08-14) — step-up = password proof (passkeys follow); no step-up over the cloudflared tunnel (it refuses password auth by design; fails closed) — passkeys supersede; mobile terminal layout is a follow-up |
