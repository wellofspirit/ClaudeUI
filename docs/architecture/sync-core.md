...
- **Killing is a first-class, VISIBLE action** — right-click a tab → "Kill terminal…" → an in-menu confirm → `terminal:kill`, and the pty's own exit closes the tab on every attached surface. Shift-click on the close button does the same without the confirm (the modifier IS the deliberateness), and the button's tooltip advertises both. The menu exists because the shortcut was invisible: detach-only closing leaves **no** discoverable UI path to stop a runaway process (a dev server, a `tail -f`) — the cold sweep only reaps cwds with no live session, i.e. never the directory you are actually working in — and an affordance nobody can find is, from the operator's chair, an affordance that does not exist. The safe action stays the unmodified one throughout: neither a plain click nor the plain menu item may take a shell away from another viewer. One implementation serves both surfaces (the web client mounts the same renderer).
