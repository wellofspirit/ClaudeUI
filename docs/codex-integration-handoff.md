...
- **F4–F7 landed 2026-09-15** (projection audit, fixture stress loop, the replica persist fence, app-server death reporting + opt-in stderr capture; Landed paragraphs in `docs/codex-followups-spec.md`). Open from them: (a) **ruling needed** — on a fresh `CODEX_HOME` ClaudeUI spawns two or three app-servers within milliseconds at boot (catalog discovery, lineage scan, auth probe) and the concurrent first-run sqlite state init makes one of them exit 1 (`failed to initialize sqlite state runtime`, captured live); serialise the first spawn when the state databases are absent, or retry once on `stdout-closed` + `exit=1` + `ready=false` (spec, F7 Landed paragraph); (b) the refactored `codex-app-server.integration.test.ts` is macOS-arm64-gated and needs one `CODEX_INTEGRATION=1` run there. The stress loop is `node scripts/codex-render-stress.mjs --load` after `bun run build`; add `CLAUDEUI_CODEX_STDERR=1` to keep the children's stderr beside the log.
