...
**Landed 2026-09-17/18 — roadmap item 2 is CLOSED.** Four commits: F21 (`57455433`) built the shared item protocol (`src/core/shared/sync/item-stream.ts`), the desktop/web transport and replica support, and Codex root/direct-child text, reasoning and plan adoption — reliable open/seal events surrounding chunk-only volatile appends, rewatch restoring the atomic active set, interruption retaining partial output. `83106588` migrated the remaining engines (Claude through `claude-item-stream.ts`, opencode through `streamItemOf`, pi through the mapper's native `contentIndex`) and every cross-engine dispatch target. `7837f7e7` retired the legacy lane: `StreamFrame`, the `text-stream` flavor, `session:stream`, `session:subagent-stream` and the four `streaming*` canonical fields are deleted, with `legacy-producer-guard.unit.test.ts` pinning that no producer emits the retired channels. `79345c43` (2026-09-18) made a Claude assistant snapshot the item lifecycle cannot place fall back to the ordinary message upsert. Design, as-built details and verification: [per-item streaming](per-item-streaming-design.md). Metering attribution and grandchild rendering are what follow.
...
- **A dispatched agent runs until the USER's limit** (2026-09-18, ADR-033's amendment of that date): the only things that end a dispatched turn early are a user stop, a caller abort, `dispatch.turnTimeoutMs`/`idleTimeoutMs`, or `dispatch.maxCostUsd`. Empty or `0` for either time limit means NO limit, in every direction (Claude, opencode, pi, Codex) — there is no built-in cap and no built-in default left to reintroduce. How MANY may run at once is a setting on the same terms: `AppSettings.dispatchMaxConcurrent` (Settings › Cross-engine dispatch › Concurrency) is read at the gate on every dispatch, so a change binds the next one — unset keeps the long-standing 3, `0` means no limit.
