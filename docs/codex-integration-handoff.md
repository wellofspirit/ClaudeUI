...
3. **Children inherit the start-time reviewer and sandbox.** `apply_spawn_agent_runtime_overrides` (codex-rs `multi_agents_common.rs`) copies the CURRENT turn's approval policy into a spawned child but the THREAD's `approvals_reviewer` and permission profile from `turn.config`, which ClaudeUI sets only at `thread/start`. A session started in Auto and switched to default therefore still hands its children's asks to the guardian (seen live 2026-09-13: the child wrote a file without asking). Fix: also push `thread/settings/update {approvalPolicy, sandbox, approvalsReviewer}` on every mode change so the baseline follows the mode; small, next after slice H (same file).
4. **opencode dispatch-target approvals carry no tool id.** The dispatcher forwards them as `dispatch:<permission>` cards without `toolUseId`, so they can only float; adding `props.tool.callID` is a small core change.
...
- No `thread/started` is ever emitted for a spawned child (three emit sites: `thread/start`, `thread/fork`, detached review). A child's early notifications must be held until its spawn item binds it. A child copies the current turn's approval policy but the thread's start-time reviewer and sandbox.
