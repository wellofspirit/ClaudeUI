...
3. **Delete-plan sweep cost.** Every Codex delete plan (including the one the confirmation opens) reads lineage for every codex `session_meta` id not in the registry, four at a time; roots never earn a registry row so they stay candidates. About 0.9 s with 25 sessions on 2026-09-13. If it grows, add a negative cache keyed on `(threadId, updatedAt)` rather than narrowing the sweep, which is what caused both delete bugs.
...
5. **Metering honesty, residual.** `SessionStatus.totalCostUsd` is a plain `number`; since `76453c7f` Codex fills it with the API-rate equivalent from published prices, so a priced model no longer reads as free. A model with no published price still shows $0 rather than \"unknown\"; making the field nullable is a cross-engine contract change and stays Daniel's call.
