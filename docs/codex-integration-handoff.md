...
## Resume here (the Codex process model — 2026-09-15)

**Read [ADR-069](adr/adr-069_codex-host-per-home-and-account.md) first.** It is the process model as built: ONE `codex app-server` per Codex home and injected ACCOUNT (`CodexHost` / `codexHostRegistry`), sessions are THREADS on it demultiplexed by `threadId`, every read borrows it, deletes go to the host that holds the thread, dispatch targets are threads, quit disposes the registry, and an ACTIVE-account switch moves the sessions that follow the active account off their host. Nothing in ClaudeUI spawns a Codex process per session or per read any more; the only app-server outside the registry is `CodexService.startLogin`'s, which must run uninjected. The slice-by-slice record — kickoffs, and a Landed paragraph per slice with the deviations — is [`codex-host-spec.md`](codex-host-spec.md) (H0 probes, H1 the host primitive, H2 sessions as threads, H3 the account switch and the writer-lock wait); ADR-069 § Probe answers and § As built carry the binary facts each slice rests on.

Facts worth knowing before touching any of it: the writer lock is PROCESS-scoped (a loaded thread refuses every other app-server's `thread/resume` and `thread/delete` with `-32600`, while the holder may delete its own); the binary unloads a thread about 60 s after its last subscriber leaves, which is what eventually releases that lock and why a cross-host resume rides a bounded retry; ending stdin exits the server in ~13 ms with an idle thread, so closing is graceful and a following start on the same home initialises cleanly; and a `disconnected` session is renderable, not a hole — a killed host's turn comes back marked `interrupted`.

### The H-series drive recipes (Windows x64, no credential, no spend)

All five live in this session's scratchpad rather than the repo, and all of them run against `scripts/codex-fixture-provider.mjs` or a small local variant of it, with `CLAUDE_UI_LOG=CodexAppServerClient,CodexHost CLAUDEUI_CODEX_STDERR=1` and the isolated-profile recipe below (`-r home-shim.cjs`, `CLAUDEUI_TEST_HOME`, `CODEX_HOME`, `CLAUDE_UI_LOG_DIR`, `CLAUDEUI_VERIFIER_HOOKS=1`):

- **D1** is in the repo: `node scripts/codex-render-stress.mjs --iterations 20 --load --home <fresh>`.
- **D2–D5** are Playwright-Electron scripts modelled on it: three concurrent sessions on one host; the ACTIVE-account switch through the real `ProviderSheet` radio with two FABRICATED vault accounts; `taskkill /F /PID <host pid>`; and six idle minutes counting spawns against poll acquires.
- **Three gotchas they cost time on.** (1) A `disconnected` status is projected by the reducer as `state: 'idle'` + `sdkActive: false` (ADR-045), so a drive must assert `sdkActive`, never the state string. (2) A session running under an INJECTED ChatGPT identity sends its model requests gzip-encoded, so a fixture that greps the prompt out of the body must decode `content-encoding` first, and it must serve `chatgpt_base_url` as well as `openai_base_url` (the binary's own backend calls go there) and accept any bearer (the fabricated JWT, not the fixture's API key). (3) Playwright must be imported from the REPO's `node_modules` by absolute path when the script lives outside the repo, and its `_electron` arrives on the CJS `default`.
- **Fabricated vault** (`<CLAUDEUI_TEST_HOME>/.claude/ui/auth-vault.json`, v3): two accounts under `accounts.chatgpt.list` with unsigned `alg: none` JWT access tokens carrying `exp`, `email` and `https://api.openai.com/auth.chatgpt_account_id`, a far-future `expires` (well inside the refresh timer's clamp, so nothing tries to refresh), made-up emails and workspace ids. Never the real vault, and never the real profile.

## Resume here (post-merge — 2026-09-15; the ADR-069 arc above supersedes its process-model notes)
...
- **Owner-found issues (Daniel, 2026-09-15, while the ADR-069 arc ran) — not yet triaged, each needs a slice:**
  1. **Codex effort cannot be changed before a session has started.** The composer's effort control on a not-yet-spawned Codex session is stuck on `low` and rejects every other value. Suspects: the pre-spawn effort path (`codexDefaultEffortFor` in `InputBox.tsx`, Slice 5b) and `CodexSession.validateEffort` needing the catalog that only exists after the first prompt; reproduce on a fresh session before touching either.
  2. **"Workspace" needs explaining.** Codex surfaces (usage, account, config) say "workspace" and it reads like a directory or project; in Codex it is the ChatGPT business workspace (`forced_chatgpt_workspace_id`, credits vs rate windows, ADR-068 findings). Decide the wording once and apply it to every Codex surface, with a one-line explanation where it first appears.
  3. **Account management is not unified across providers.** Anthropic accounts have a dedicated Accounts section; ChatGPT accounts are reached through "manage" on the ChatGPT subscription and then switched. Either every provider's accounts live in the Accounts section or every provider's live in its subscription sub-panel; one vocabulary, one place (ADR-065 rules, ADR-068 §3 entry points).
  4. **Only one ChatGPT account is displayed at a time.** With two accounts in the vault the list shows one; removing it makes the other appear. A bug in the accounts list rendering or the `provider-account:list` projection (Slice 1/2b), not a design question; needs a guard test with two fabricated accounts.
  5. **Codex offers the model the desktop app's browser tools and believes it is inside the Codex desktop app.** Seen live 2026-09-15: a turn tried to verify a browser build through an "in-app browser" ClaudeUI does not have. Root cause is the binary's feature table, not our launch parameters: in 0.154 `browser_use`, `browser_use_external`, `browser_use_full_cdp_access`, `computer_use` and `in_app_browser` (plus `in_app_chat`, `in_app_dictation`, `in_app_local_automation`, `in_app_updates`) are `Stage::Stable` with `default_enabled: true` (`codex-rs/features/src/lib.rs`), so a real `config.toml` that does not disable them lets the model see tools only the Codex desktop app can serve; the fixture homes disable exactly these under `[features]`, which is why no drive ever showed it. Fix shape: pass per-thread config overrides on `thread/start`/`resume`/`fork` that turn off every desktop-app-only feature ClaudeUI cannot host (the same override channel Slice 4 uses for `mcp_servers`), never by writing the user's file; inventory the feature table against what ClaudeUI actually hosts (hosted tools, MCP, dispatch) before choosing the list, and add a guard that the thread's effective feature set contains nothing unhosted.
  6. **No reasoning shows for Codex turns.** The pipeline is wired: `mapCodexDelta` turns `item/reasoning/summaryTextDelta` and `item/reasoning/textDelta` into thinking stream deltas and a completed `reasoning` item into a thinking block (`event-mapper.ts`), the generator selects both notifications, and `CodexSession.notification()` routes deltas through the mapper; Codex's own defaults leave summaries on (`model_reasoning_summary` defaults to `auto`, `hide_agent_reasoning` to `false`, `codex-rs/config/src/config_toml.rs`). So one of two things: (a) the wire carries no reasoning items for the model and account in use — check the user's `config.toml` for `model_reasoning_summary = "none"` or `hide_agent_reasoning = true`, and whether the ChatGPT backend returns summaries for the GPT-6 family at all (a debug line per `item/reasoning/*` notification would settle it in one turn); or (b) the renderer hides thinking for Codex sessions (the thinking block or its capability gate); reproduce with a real turn on `gpt-5.6-luna` versus `gpt-6-astra` before touching either side. Owner screenshot 2026-09-15 narrows it: the transcript shows collapsed "Thought" blocks with NOTHING inside them after the guardian row, so `reasoning` items DO arrive and the mapper renders them, but with empty `summary` and `content` (`event-mapper.ts` joins whichever is non-empty). That points at the wire: the ChatGPT backend returns reasoning items without summary text for that model unless the request asks for one (`model_reasoning_summary`, the `reasoning.summary` request field), not at a renderer gate. Two fixes: skip rendering a reasoning item whose summary and content are both empty (a "Thought" with nothing in it is noise), and expose `model_reasoning_summary` on the Codex settings page so the user can ask for `detailed`. Worth a Codex settings row for `model_reasoning_summary` once the cause is known.
  7. **The auto-review verdict reads like reasoning; real reasoning shows nothing.** The guardian verdict is emitted by `CodexSession.guardianRow` as a `system` message with plain text (`Codex auto-review approved `…` (risk: medium). …`), rendered as a quoted block right where a thought would sit, while the thinking blocks beside it are empty (item 6). Rule to decide: the verdict is a permission decision, not model reasoning, and should render with the vocabulary of the approval cards (decision, risk, rationale), never as prose in the reasoning slot; the reasoning slot shows reasoning or nothing.
  8. **Show the judge's verdict on the tool card it judged.** For Codex, `item/autoApprovalReview/completed` carries the reviewed `action` and the review's target item; today it becomes a separate row (item 7) and only a DENIAL is bound to its card (the override on the declined card, ADR-067 amendment). Bind every completed review to the tool card of the item it reviewed and render the decision, risk and rationale on that card (approved rows included), keeping the standalone row only for reviews with no target item (`guardianWarning`, the breaker). Do the same for ClaudeUI's own Auto-mode judge on Claude, opencode and pi tool cards, so an auto-approved command shows who approved it and why on the card itself. Renderer and mapper work; the Codex half needs the `targetItemId` binding the declined-card override already uses.
  9. **Remote VS Code no longer launches on this Windows host** (Daniel, 2026-09-15; it worked before). ADR-064: `code serve-web` is proxied under `/vscode` behind the `ide` capability, tailnet and localhost only (`src/core/services/vscode-web-service.ts`, `remote-server.ts`, `host-anchor.ts`). Leads, in the order to check: the `code` CLI locator on Windows (the executable is `code.cmd`, and the last touch here was the PR #34 "vscode probe path flavour fix" — a Git Bash versus Windows path-flavour regression is the obvious suspect since this branch's bash-flavoured work); the serve-web probe facts recorded in ADR-064 (the WebSocket upgrade is ungated, the cookie is Host-bound, `remoteAuthority` follows `Host`), which break if the tailnet name or port changed; and the main log for the probe's failure line when the IDE button is pressed. Sharper lead from the code: on Windows `probeCli` auto-probes ONLY `.exe` candidates — `code-tunnel.exe` beside whatever `code` resolves to on the APP's PATH, then the platform well-known install locations — because `code.cmd` is an unspawnable batch wrapper (`vscode-web-service.ts` ~`:366-381`), and a candidate counts only if `<candidate> serve-web --help` exits 0 within the probe timeout. So the first checks are: does the desktop app's own PATH (not Git Bash's) still reach `code`, is `code-tunnel.exe` still beside it after a VS Code update or a user-scope reinstall, and does `code-tunnel.exe serve-web --help` exit 0 by hand; the explicit CLI override in the IDE settings is exempt from the `.exe` rule and is the immediate workaround. Reproduce from a phone or a second machine over the tailnet against the running desktop app, read the log's probe line, then decide.

- **ADR-069 accepted 2026-09-15 (Codex host per home and account).** Implementation runs from `docs/codex-host-spec.md`: H0 probes P1–P4 on the real binary, H1 host primitive + reads on the host, H2 sessions as threads, H3 account switch / recycle / quit / E2E. F10 (catalog memo per binary and account) landed as ec60d1e5.
- **F4–F9 landed 2026-09-15** (projection audit, fixture stress loop, the replica persist fence, app-server death reporting + opt-in stderr capture, the first-run spawn gate, caller labels on every spawn; Landed paragraphs in `docs/codex-followups-spec.md`). Open from them, **ruling needed** (F9 Landed paragraph): the first session's app-server dies on an initialised home when it starts as the composer's per-cwd catalog discovery one-shot is force-killed; three independent parts to rule on — memoise discovery per binary, graceful dispose + a shutdown-covering gate versus retry-once, and the 30-second directory poll that spawns an app-server each time. Still open: the refactored `codex-app-server.integration.test.ts` is macOS-arm64-gated and needs one `CODEX_INTEGRATION=1` run there. Diagnostics: `node scripts/codex-render-stress.mjs --load` after `bun run build`; add `CLAUDEUI_CODEX_STDERR=1` to keep the children's stderr beside the log and `CLAUDE_UI_LOG=CodexAppServerClient` to see every spawn with its caller label.
...
4. **Windows residuals: DONE 2026-09-14** (real turns on the owner's account; findings recorded in `docs/architecture/codex.md`, transport section). There is no Windows sandbox in Codex 0.154.0; default mode prompts as everywhere, and Auto sends EVERY command and patch through the guardian (`exec_policy.rs` returns `Prompt` when the Windows sandbox backend is disabled), seen live as one "auto-review approved" row per command. Process-tree cleanup held with one and with two sessions closed mid-`ping`: no `codex`, `codex-code-mode-host`, `ping` or Codex `pwsh` survived the quit.
5. **Real-account evidence for the newest slices** — Daniel's, because each spends the real account: one Codex turn started on a configured default model (Slice 5b; the wire is pinned by test up to `session:create`), and one real device-code sign-in completed from a phone (Slice 7; everything up to the code being shown is proven live, nothing was completed). DONE on 2026-09-14 by a different route: a real Codex turn from the WEB client against the Linux headless server (M5-L Landed paragraph in the spec), which found and fixed the JSON-`null` optional-argument bug on remote `session:create`.
6. **`--disable-auth` and the web client (found 2026-09-14).** With auth policy `off` and no break-glass password, `/remote/auth-info` advertises no method and the web client dead-ends at "Missing Token"; the documented headless bypass has no way in from a browser. Decide whether the client should get an `off` route or the flag should require a password.
7. **Decide whether the history mappers block the merge.** Cold history omits several native item kinds (roadmap item 1). Recommendation: merge first, mappers next; Daniel's call.
...
- **Server builds for macOS and Windows (Daniel, 2026-09-14): follow-up, not part of the Linux leg.** Today `claudeui-server` is compiled and released only on the two Linux legs of `pre-release.yml` and `release.yml`. Daniel wants the same tarball produced for macOS arm64 and Windows x64. Inputs when it is taken: `scripts/build-server.mjs` already accepts a bun compile target (`--target=bun-darwin-arm64`), `verify:sqlite` runs before every server build, the desktop matrix jobs already have the vendored engine binaries for those hosts (including the patched `bun-claude`, which Linux lacks), and the assemble step must copy `vendor/codex-cli` the same way the Linux legs do after M5-L.
