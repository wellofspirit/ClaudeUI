...
3. **Interrupted hosted-tool call, decision pending.** A `render_mermaid`/mockup call interrupted mid-flight is absent from the turn's items in Codex's stored history, and live the card ends with no result at all (pinned by `codex-interrupted-tool.integration.test.ts`, `cf11b983`). The spec's presentation supplement was not built. Cheapest useful fix: on an interrupted turn, synthesize a failed tool_result for any in-flight dynamic-tool block so the live card closes; cold history would still omit it unless a supplement is persisted. Daniel's call.
