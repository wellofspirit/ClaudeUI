...
Not built: archive (unused by decision), a cold-history supplement for interrupted hosted-tool calls (decided against; the live card is closed instead), full metering, Windows and Linux digests and packaging (the acquisition skips there; Daniel's, Windows first).
