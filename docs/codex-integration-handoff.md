...
1. **F19 — reasoning summaries (item 6b, closed as a ruling, reopened as two small fixes).** Probed live 2026-09-16: with `detailed` the ChatGPT backend sends one bold-Markdown headline per reasoning item (`summaryPartAdded`, one `summaryTextDelta`, `summary: ["**…**"]`); with the catalog default `none` it sends an empty reasoning item and no deltas. The setting resolves per thread at open; an already-open session keeps its value. The settings row exists (Codex page › Model behaviour) and ClaudeUI adds no per-thread override. Fixes: strip the bold wrapper, a helper sentence on the row, a fixture `--reasoning` option. The empty "Thought" in the owner's screenshots matches the pre-F13 mapper; retest on a rebuilt app with a NEW session. Kickoff in `codex-followups-spec.md`.
