...
- **F4–F8 landed 2026-09-15** (projection audit, fixture stress loop, the replica persist fence, app-server death reporting + opt-in stderr capture, the first-run spawn gate; Landed paragraphs in `docs/codex-followups-spec.md`). Open from them: (a) **ruling needed** — on an INITIALISED home the first session's app-server can still die with `failed to initialize sqlite state runtime` when it starts as the auth probe's short-lived app-server exits (F8 Landed paragraph lists three candidate fixes; widening the gate to every start was tried and reverted because it made the collision deterministic); (b) the refactored `codex-app-server.integration.test.ts` is macOS-arm64-gated and needs one `CODEX_INTEGRATION=1` run there. The stress loop is `node scripts/codex-render-stress.mjs --load` after `bun run build`; add `CLAUDEUI_CODEX_STDERR=1` to keep the children's stderr beside the log.
