...
| `account_label`     | What a person calls the account (the email, or `OpenRouter key …a41f`). Display only.                                                                                     |
...
| `billed_cost_usd`   | Money that left a wallet. The engine's figure for `apiKey`, `0` for `subscription` and `free`, null for `unknown`.                                                        |
