...
| `voice` (ADR-077)                    | `true` | `false`  | The static Claude flag is `true`, but the voice server is ClaudeUI's own cli.js patch, so the HONEST per-session value is `static AND harnessHasPatch('voice-server')` (`ClaudeSession.capabilities`, read from the `version.json` beside the spawned binary; Anthropic's unpatched binary reads as no patches). Every mic gate — InputBox button and Tab shortcut, `voice:start-*` IPC, remote `voice:start` — reads that one value.                                                                                                                                                                                                                                                                                                                                                                                            |
