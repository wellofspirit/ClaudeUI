...
- **The paste-field UI is a separate follow-on.** This series builds the flow and its tests. Delivering `manualUrl` / a paste field / the code-entry box to the remote client, and wiring the `auth:state` event to a remote connection, is S4-UI.
