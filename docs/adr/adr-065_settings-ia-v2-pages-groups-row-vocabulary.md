...
**Status:** **Implemented** (2026-09-08); amended by [ADR-068](adr-068_chatgpt-identity-vault-owned-codex-injection.md) (ChatGPT row gains accounts + a Codex route; Codex engine page curated; Codex segments on judge / Default models / Dispatch) and by [ADR-074](adr-074_provider-surfaces-v3.md) (the one Providers list splits into Subscriptions and API providers; the Accounts group folds into Subscriptions; the Anthropic endpoint moves to the Claude page) — the seven phases below landed on branch `settings-v2`; see § As built for what the implementation changed about this decision.
