...
1. **Closed wire-contract set.** Every interaction is exactly one of: **command** (validated, capability-checked, identity-attached, audited; never mutates client state), **domain event** (`{seq,…}`, append → apply → broadcast; the _only_ replicated-state mutation path), **volatile stream** (streaming deltas / PTY bytes; subscription-scoped, never logged — their coalesced value lives in canonical state, with `{turnId, offset}` self-healing), or **query** (RPC read). No fifth path.
