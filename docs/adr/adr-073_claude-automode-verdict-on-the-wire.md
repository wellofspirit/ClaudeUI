...
### 6. Subagent decisions are dropped at the producer
...
with a debug log rather than emitted-and-silently-discarded, so the boundary is
explicit. Wiring it needs a `session:subagent-tool-review` channel and reducer
case; `taskIdMap` (agentId → parent toolUseId) already exists.

## Three wire facts this ADR exists to stop anyone rediscovering
...
- **Open gap at acceptance**: a classifier _denial_ was verified on the wire and
  in unit tests, but never rendered in the live app — the judge allowed every
  safe probe the verifier tried, and escalating to a genuinely unsafe command
  to get a screenshot was correctly refused. The `denied` + rule-badge path is
  therefore test-covered only.
