...
Amended 2026-09-21 by S5a as built: §2 (the high-water mark is `usage_event`'s rowid and starts at the moment sync is enabled), §3 (the local `remote_*` tables and how a changed `epoch` is handled), §5 (the exact per-event field list, and the device facts each push carries), §6 (**the device credential lives in the operational database, not the vault**, and a rejected credential is a 302 or a 401), §7 (the row-written notifier, the debounce, the timers, the state vocabulary, `redirect: 'manual'`) and §8 (where the draft protocol lives, and `SCHEMA_VERSION 1`).
...
**The high-water mark is `usage_event`'s hidden `rowid`, and it starts at the moment sync is enabled (amended 2026-09-21, S5a).** Neither of the two obvious candidates works: `id` is a random uuid, so it orders nothing, and `ts` is not monotonic, because the reconciler backfills turns from transcripts every ten minutes and a row written now can carry yesterday's timestamp. `rowid` rises with every insert, and the 90-day prune deletes the OLDEST rows, so its maximum is never reused. It lives in `usage_hub_config.cursor_rowid`.

Enabling sync on a machine initialises the cursor to `MAX(rowid)` (owner: start fresh from the trusted source), so nothing written before that moment is ever pushed. An `unknown` row is READ and skipped rather than filtered out in SQL: the cursor has to be able to move past it, and a row the query hid would be a rowid the cursor could never pass.
...
| --------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A device  | An Access service token, sent as `CF-Access-Client-Id` and `CF-Access-Client-Secret`, under a Service Auth policy | Write `POST /v1/events` and `POST /v1/limits`, and resync itself. Read the aggregates (`/v1/buckets`, `/v1/windows`, `/v1/limits`) with account labels masked, **and the device list** (`/v1/devices`: `deviceId`, `deviceName`, `os`, `appVersion`, `lastPushAt`, `retired`). No owner writes, no raw events, no unmasked label. |
