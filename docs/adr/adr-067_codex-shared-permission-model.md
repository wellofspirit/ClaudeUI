...
- Under `auto`, Codex's guardian sees in-workspace commands that ClaudeUI's rules never see. A user-authored deny rule does not fire for a command the guardian approves inside the sandbox. This is the accepted meaning of "leave Auto to their native judge".
- Every `untrusted` command costs one round trip to the client. A later optimisation may compile Claude **allow** rules into a ClaudeUI-owned `~/.codex/rules/claudeui.rules` so Codex runs them without asking; that would not add containment (explicit execpolicy allows also bypass the sandbox) and is lossy (glob to argv prefix), so it is an optimisation, not a safety feature, and is not part of this decision.
- The `session:codex-settings` channel now has no UI caller (model and effort travel over the shared channels); its explicit `reset` remains reachable only programmatically.
